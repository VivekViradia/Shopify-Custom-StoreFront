...
// Create a cart with Lines mutation
...
export async function cartRemove({cartId, lineIds, storefront}) {
  const {cartLinesRemove} = await storefront.mutate(REMOVE_LINE_ITEM_MUTATION, {
    variables: {
      cartId,
      lineIds,
    },
  ...
  if (!cartLinesRemove) {
    throw new Error('No data returened from remove lines mutation');
  }
...
@inContext(country:$country, language:$language){
    cartLinesAdd(cartId, lines:$lines){
        cart{
            ...CartLinesFragment
        }
        errors: userErrors{
...
const REMOVE_LINE_ITEM_MUTATION = `#graphlql
mutation($cartId:ID!, $lineIds:[ID!]!, $langauge:LanguageCode, $country: CountryCode)
@inContext(country: $country, language:$language){
